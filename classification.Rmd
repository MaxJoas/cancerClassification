...
The aim is to only give a very short overview, so the reader undserstands the key terms of the methods.
Random forest is a widely used method for classification. The base of a random forest model is 
are decision trees. A decision tree consits of multiple nodes. At each node tree is splitted into 
branches depending on the value of a indepented variable. In our case this would be the expression level
of a particular gene. Which variable is used at a given split is random. However not all variables are available to sample from at each split. The number of variables available for splitting at each tree node is referred to as the mtry parameter . The end of a branch, the leaf, does not split anymore and represents the decision for a particlaur class. In our case that would be a cell type. 
A random forest consists of multuple decision trees. Each tree gives a vote for the final decision of 
the class. The class with the majority vote across all trees is the final class of a random forest.
The number of trees used in a random forest is on interest in this report. I will investigate the 
influence of the number of trees on the classification accuracy.


SVM is another well estbalished method for classification it tries to seperate the data by a hyperplane.
The goal is that datapoints in different classes are as far away from the hyperplane as possible. The
datapoints nearest to the hyperplan are called support vectors. Often times the datapoints are not seperatable in their original dimeninsion. Nevertheless it is always possible to seperate the data in a higher dimension. Therefore we need a way to map the datapoints to a higher dimension. The function used for this process is called Kernel function. There are different types of Kernels and I will investigate
the influence of the Kernel function on the classification accuracy.

In microarray studies it is common to have a high number of measurements in comparision to the number of
patients. This is called a small n large p dataset and can lead to overfiting. Therefore, it can make
sense to select only a subset of independend variables. Ideally the ones that contain the most information. In order to find these variables I also used random forest for feature selection. Precisely
...
implementation of the above described methods. In order to test the classifaction accuracy I need to 
...
The base function for calssification take the parameters for the corresponding method, e.g. number of trees for random forest and the Kernel method for SVM.

For the feature selection I applied the same principles I hade one function that did the actual feature selection and one function that implements the LOOCV logic and calls the feature selection function. I is also possible to specifiy how many feature shoud be selected. However I did not want to chose the number of features arbitraly. So I sorted the variable importance of all covariates and plotted it. Subsequently, I smoothed the plot in order to make it differentiable. The goal was to find the point where the variable importance drops the steepest. This point I used as a cutoff for the number of features that I selected. In order to find the point I took the highest value of the second derivative
of the smoothed line.
All the code can be found in the Appendix section.
