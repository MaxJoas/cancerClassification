...
#load(memImageFile) error attemtp to use zero length variable name
...
branches depending on the value of a independent variable. In our case this would be the expression level
...
patients. This is called a small n large p dataset and can lead to overfitting. Therefore, it can make
sense to select only a subset of independent variables. Ideally the ones that contain the most information. In order to find these variables I also used random forest for feature selection. Precisely
...
plot(sort(x, na.last = TRUE, decreasing = TRUE), ylab = "Variable Importance")
...
I will not go into detail in this part[@den2009subtype], since it is basically the same code that we did @den2009subtype
together in class. I will only comment on things that I have changed in comparison to the work in class.
The main difference in this section is that I registered a parallel backend for multithreading and that 
...
#plot(sort(x[1:200],  na.last = TRUE, decreasing = TRUE))
lo <- loess(x[1:preFiltered] ~ c(1:200))
out = predict(lo)
secondDer <- diff(diff(out))
maximalChangePoint <- max(secondDer)
maximalChangeIndex <- match(maximalChangePoint, secondDer)
...
plotData <- data.frame(`Variable Importance` = x[1:preFiltered],
...
ggplot(plotData,
       ...
       geom_line(aes(y = out, color = "smoothed"))  +
       geom_vline(aes(xintercept = maximalChangeIndex, color= "Turning Point"), size = 1.5) 
...
parameters <- c("RF 200 Trees all Genes", 
                ...
                "RF 200 Trees Selected Genes", 
                "RF 500 Trees Selected Genes", 
                "RF 1000 Trees Selected Genes", 
                "SVM Radial Kernel all Genes",
                "SVM Linear Kernel all Genes",
                "SVM Ploynomial Kernel all Genes",
                "SVM Sigmoid Kernel all Genes",
                "SVM Radial Kernel Selected Genes",
                "SVM Linear Kernel Selected Genes",
                "SVM Ploynomial Kernel Selected Genes",
                "SVM Sigmoid Kernel Selected Genes")
...
evaluationResults <- foreach(i = c(1:length(resultVector)), .combine = 'rbind') %do% {
  res <- resultVector[[i]]
  cm <- confusionMatrix(res, response)
  # extract the confusion matrix values as data.frame
  cm.table <- as.data.frame(cm$table)
  # confusion matrix statistics as data.frame
  cm.stats <-data.frame(cm$overall)
  # round the values
  cm.stats$cm.overall <- round(cm.stats$cm.overall,2)
  ...
  geom_tile() +
  geom_text(aes(label = paste("",Freq,",",Perc,"%")), color = 'white', size = 4, fontface = "bold") +
  theme_light() +
  guides(fill=FALSE) 
  ...
  # all together
  par(mfcol = c(4, 4))
  finalPlot <- grid.arrange(cm.plot, cm.statsTable,nrow = 1, ncol = 2, 
             top=textGrob(names(resultVector)[i],gp=gpar(fontsize=20,font=1)))
  ...
  misclError <- sum(diag(tab))/sum(tab)
  ci.upper <- misclError + 1.96 * 
              sqrt( (misclError * (1 - misclError)) / nrow(exprTableTransposed))
  ci.lower <- misclError - 1.96 *
              sqrt( (misclError * (1 - misclError)) / nrow(exprTableTransposed))
